...
meshcortex/
  packages/
    ...
    backends/           # Grouping folder for inference-node packages (no shared code/deps)
      ...
      edge-node/        # Edge inference backend — own pyproject.toml, ARM deps
...
Each leaf package (`orchestrator`, `gpu-node`, `edge-node`, `mcp-layer`) is independently installable/testable, but the whole repo shares one version history, one CI pipeline, and one set of lint/format rules. This replaces the earlier multi-repo plan (separate repos synced via version tags).

`backends/` is not a shared package: `gpu-node` and `edge-node` each keep their own `pyproject.toml` because their dependencies are incompatible (CUDA/vLLM vs. ARM/CPU-only llama.cpp — neither installs on the other's machine). They do share a Python namespace package, though (PEP 420): `gpu-node` is imported as `backends.gpu_node`, not as a top-level module, reflecting that both are "model-running nodes," conceptually separate from `orchestrator` and `mcp-layer`. Model weights are never committed: each node downloads/caches its own weights locally (gitignored), keyed against the single shared manifest in `configs/models.yaml`.
