...
`backends/` is a grouping folder, not a shared package: `gpu-node` and `edge-node` each keep their own `pyproject.toml` because their dependencies are incompatible (CUDA/vLLM vs. ARM/CPU-only llama.cpp — neither installs on the other's machine). Grouping them under `backends/` just reflects that both are "model-running nodes," conceptually separate from `orchestrator` and `mcp-layer`. Model weights are never committed: each node downloads/caches its own weights locally (gitignored), keyed against the single shared manifest in `configs/models.yaml`.
